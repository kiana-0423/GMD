...
| **Constraint-rank fallback and tie-breaking coverage** — the rank cross-check fallback is exercised end to end through the real analysis pipeline by a near-coplanar fixture, and the relative near-tie window is proven necessary by a rotated fixture whose tied residual norms are not bit-identical | `tests/constraint_rank_fallback_tests.cpp` `tests/constraint_tie_breaking_tests.cpp` `tests/mpi_constraint_rank_fallback.cpp` |
